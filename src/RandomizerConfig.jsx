...
function ViewVersion() {
  ...
  return (
    <>
      ...
      {loading ? <span>Loading...</span> : <p>Algorithm version: {data}</p>}
    </>
  );
}
...
export default function RandomizerConfig() {
  ...
  return (
    <ErrorBoundary>
      <p className="error">{error?.message}</p>
      <p>Prospective Randomizer at: {randomizerURL}</p>
      <ViewVersion />
      <RefreshContext.Provider value={refetch}>
        {loading ||
          (data &&
            Object.getOwnPropertyNames(data).map((p) => (
              <ViewProtocol
                key={p}
                name={p}
                protocol={data[p]}
